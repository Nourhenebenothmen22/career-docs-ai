...
exports.generate = async (req, res, next) => {
  try {
    const result = await motivationService.generate(req.body, req.user?.id);
...
exports.downloadPdf = async (req, res, next) => {
  try {
    const result = await motivationService.generateWithPdf(req.body, req.user?.id);
    if (!result.pdfBuffer && !result.pdfStream) {
...
exports.generateStream = async (req, res, next) => {
  try {
    ...
    const prompt = promptBuilder.buildMotivationPrompt(enrichedData);
    ...
    const htmlContent = htmlTemplate.motivationLetter(enrichedData, fullText);
